  ...
  return () =>
    new Promise((resolve, reject) => {
      ...
      const guardReturn = guard.call(
        ...
        to,
        from,
        true ? canOnlyBeCalledOnce(next, to, from) : next,
      );
...
function extractComponentsGuards(matched, guardType, to, from) {
  ...
  for (const record of matched) {
    if (!record.components && !record.children.length) {
      warn(
        `Record with path "${record.path}" is either missing a "component(s)" or "children" property.`,
      );
    }
    for (const name in record.components) {
      ...
      if (true) {
        if (
          ...
          warn(
            `Component "${name}" in record with path "${
              record.path
            }" is not a valid component. Received "${String(rawComponent)}".`,
          );
      ...
      api.on.getInspectorState((payload) => {
        if (payload.app === app && payload.inspectorId === routerInspectorId) {
          ...
          const route = routes.find(
            (route2) => route2.record.__vd_id === payload.nodeId,
          );
          if (route) {
            payload.state = {
  ...
  function resolve(rawLocation, currentLocation) {
    ...
    if (typeof rawLocation === "string") {
      ...
      if (true) {
        if (href2.startsWith("//"))
          warn(
            `Location "${rawLocation}" resolved to "${href2}". A resolved location cannot start with multiple slashes.`,
          );
